...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Transferencia') }}</div>

                <div class="card-body">
                  <form action="{!! route('cuentacorriente.store') !!}" method="post">
                        @csrf
                    <input type="hidden" name="tipo_movimiento" value="transferencia"/>
                    ...
                    <div class="form-group row">
                        <label for="name" class="col-md-4 col-form-label text-md-right">{{ __('Cuenta Destino') }}</label>

                        <div class="col-md-6">
                            <select name="cuenta_destino_id" class="form-control{{ $errors->has('cuenta_destino_id') ? ' is-invalid' : '' }}" autofocus required>
                              <option value="">
                                Seleccione cuenta
                              </option>
                            ...
                            @if ($errors->has('cuenta_destino_id'))
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $errors->first('cuenta_destino_id') }}</strong>
                                </span>
                            @endif
                        </div>
